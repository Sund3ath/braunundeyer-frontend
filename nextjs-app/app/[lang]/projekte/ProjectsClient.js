...
export default function ProjectsClient({ initialProjects = [], dict = {} }) {
  ...
  const projects = initialProjects.length > 0 ? initialProjects : [
    ...
    // ... keep other fallback projects as needed
  ];
  ...
  const filteredAndSortedProjects = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'Älteste':
          return (a.year || 0) - (b.year || 0) || (a.id || 0) - (b.id || 0);
        case 'A-Z':
      ...
      <main className="pt-20 lg:pt-24">
        ...
        <section className="bg-surface/95 backdrop-blur-sm border-b border-border relative z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
            <Breadcrumb items={[
              { label: dict?.translation?.nav?.home || 'Home', href: `/${lang}` },
            ...
            <div className="mt-6">
              <h1 className="text-3xl lg:text-4xl xl:text-5xl font-heading font-light text-primary mb-4">
                {dict?.projects?.title || 'Projektgalerie'}
              </h1>
              <p className="text-lg text-text-secondary font-body max-w-2xl">
                {dict?.projects?.subtitle || 'Entdecken Sie unser umfassendes Portfolio architektonischer Projekte von Neubau über Altbausanierung bis hin zur Innenarchitektur.'}
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            ...
            <div className="mb-4 lg:mb-6">
              <div className="relative max-w-md">
                ...
                <input
                  type="text"
                  placeholder={dict?.projects?.searchPlaceholder || "Projekte suchen..."}
                  value={searchQuery}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
              ...
              <div className="flex items-center space-x-4">
                ...
                {(selectedCategory !== 'Alle' || searchQuery) && (
                  ...
                  >
                    {dict?.projects?.clearFilters || 'Alle löschen'}
                  </button>
                )}
                ...
                <div className="relative">
                  ...
                  >
                    <option value="Neueste">{dict?.projects?.sortNewest || 'Neueste'}</option>
                    <option value="Älteste">{dict?.projects?.sortOldest || 'Älteste'}</option>
                    <option value="A-Z">{dict?.projects?.sortAZ || 'A-Z'}</option>
                  </select>
                  <ChevronDown 
                    size={16} 
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="mb-6 lg:mb-8">
              <p className="text-text-secondary font-body">
                {dict?.projects?.showing || 'Zeige'} {displayedProjects.length} {dict?.projects?.of || 'von'} {filteredAndSortedProjects.length} {dict?.projects?.projects || 'Projekten'}
              </p>
            </div>
            ...
            {filteredAndSortedProjects.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-heading font-light text-primary mb-2">
                  {dict?.projects?.noProjectsFound || 'Keine Projekte gefunden'}
                </h3>
                <p className="text-text-secondary font-body mb-6">
                  {dict?.projects?.tryAdjustingFilters || 'Versuchen Sie, Ihre Suchkriterien oder Filter anzupassen'}
                </p>
                <button
                  onClick={handleClearFilters}
                ...
                >
                  {dict?.projects?.clearFilters || 'Filter löschen'}
                </button>
              </div>
            ) : (
              <>
                          ...
                          <div className="md:w-2/3 p-6 lg:p-8">
                            ...
                            <div className="flex items-center justify-between text-sm text-text-secondary font-body">
                              ...
                              <div className="flex items-center space-x-1 text-accent">
                                <span>{dict?.projects?.viewDetails || 'Details anzeigen'}</span>
                                <ArrowRight size={16} />
                              </div>
                            </div>
                          </div>
                    ...
                    >
                      {isLoading ? (
                        <div className="flex items-center space-x-2">
                          ...
                          <span>{dict?.projects?.loading || 'Lädt...'}</span>
                        </div>
                      ) : (
                        dict?.projects?.loadMore || 'Weitere Projekte laden'
                      )}
                    </button>
                  </div>
                )}
