...
export const projectsAPI = {
  ...
  unpublish: async (id) => {
    const response = await fetch(`${API_BASE_URL}/projects/${id}/unpublish`, {
      method: 'POST',
      headers: getAuthHeaders(),
      credentials: 'include'
    });
...
export const mediaAPI = {
  ...
  getAll: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    const response = await fetch(`${API_BASE_URL}/media${queryString ? `?${queryString}` : ''}`, {
      headers: getAuthHeaders(),
      credentials: 'include'
    });
    return handleResponse(response);
...
export const translationsAPI = {
  ...
  bulkTranslate: async (texts, targetLanguages, sourceLanguage = 'de') => {
    const response = await fetch(`${API_BASE_URL}/translate/bulk`, {
      method: 'POST',
      headers: getAuthHeaders(),
...
export const auditAPI = {
  getLogs: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    const response = await fetch(`${API_BASE_URL}/audit${queryString ? `?${queryString}` : ''}`, {
      headers: getAuthHeaders(),
      credentials: 'include'
    });
    return handleResponse(response);
